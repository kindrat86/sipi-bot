...
class Handler(BaseHTTPRequestHandler):
  ...
  <text x="{width-16}" y="118" fill="#8a8d96" font-family="SF Mono,ui-monospace,monospace" font-size="8" text-anchor="end">sipi.bot/badge</text>
    ...
    _PSEO_301_REDIRECTS = {
        # W9 (2026-08-08 GSC "Duplicate, Google chose different canonical than
        # user") — the two /integrations/sipi-bot-plus-* pages were generated by
        # lib/generate_pseo_round21.py from a single template. Their bodies were
        # 98.4% identical to EACH OTHER and ~78% identical to the short-slug
        # vendor page they shadow (/integrations/stripe, /integrations/openai),
        # which carries the same enrich block appended verbatim. They shared an
        # <h1> with the short-slug page too. Nothing on them was unique, so
        # Google overrode their self-canonical. Both pages are retired: the
        ...
        # FAQ). It used to 301 into the retired sipi-bot-plus-* page above;
        # it now points at /integrations/openai-agents/, the real Agents SDK
        # walkthrough (setup, testing, pricing — content the retired page
